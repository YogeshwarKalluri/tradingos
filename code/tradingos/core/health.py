...
from tradingos.core.config import HealthConfig, get_config
...
def create_health_app(config: HealthConfig | None = None) -> FastAPI:
    """Create health check FastAPI app."""
    server = HealthServer(config)
    return server.app


async def run_health_server(config: HealthConfig | None = None):
